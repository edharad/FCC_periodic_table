...
--
-- Name: capitalize_first_letter(text); Type: FUNCTION; Schema: public; Owner: freecodecamp
--

CREATE FUNCTION public.capitalize_first_letter(input text) RETURNS text
    LANGUAGE plpgsql
    AS $$
BEGIN
...
RETURN NULL;
END IF; 
RETURN UPPER(SUBSTRING(input FROM 1 FOR 1)) || SUBSTRING(input FROM 2);
END; 
$$;


ALTER FUNCTION public.capitalize_first_letter(input text) OWNER TO freecodecamp;
